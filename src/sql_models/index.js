...
export default function initModels(sequelize) {
  ...
  // === NUEVAS ASOCIACIONES ===
  // Usuario -> Paciente (un usuario tiene un perfil de paciente)
  Usuario.hasOne(Paciente, { foreignKey: 'usuario_id' })
  Paciente.belongsTo(Usuario, { foreignKey: 'usuario_id' })

  // Usuario -> Doctor (un usuario tiene un perfil de doctor)
  Usuario.hasOne(Doctor, { foreignKey: 'usuario_id' })
  Doctor.belongsTo(Usuario, { foreignKey: 'usuario_id' })
  
  // === ASOCIACIONES EXISTENTES (MODIFICADAS) ===
  // Doctor -> Especialidad (un doctor pertenece a una especialidad)
